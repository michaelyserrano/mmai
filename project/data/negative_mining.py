...
def build_random_negatives(api_corpus: list[dict], ground_truth_apis: list[str], n: int = 99) -> list[dict]:
    ...
    pool = [a for a in api_corpus if a["name"] not in ground_truth_apis]
...
def build_category_sibling_negatives(
    ...
    pool = [
        ...
        if a["category"] in gt_categories and a["name"] not in ground_truth_apis
    ]
    if len(pool) == 0:
...
def build_dfsdt_negatives(
    ...
    if len(unique_failures) < n:
        siblings = build_category_sibling_negatives(
            api_corpus, ground_truth_apis + [a["name"] for a in unique_failures],
...
def build_api_lookup(api_corpus: list[dict]) -> dict[str, dict]:
    ...
    return {a["name"]: a for a in api_corpus}
...
if __name__ == "__main__":
    ...
    assert not any(a["name"] in gt for a in randoms)

    siblings = build_category_sibling_negatives(corpus, gt, lookup, n=9)
    assert isinstance(siblings, list) and len(siblings) <= 9 and not any(a["name"] in gt for a in siblings)
    assert not any(a["name"] in gt for a in siblings)
